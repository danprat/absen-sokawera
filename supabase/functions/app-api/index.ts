...
function stripRoute(url: URL) {
  const marker = "/functions/v1/app-api";
  if (url.pathname.startsWith(marker)) return url.pathname.slice(marker.length) || "/";
  const runtimeMarker = "/app-api";
  if (url.pathname.startsWith(runtimeMarker)) return url.pathname.slice(runtimeMarker.length) || "/";
  return url.pathname;
}
...
async function handle(req: Request) {
  ...
  if (path === "/health") return json({ status: "healthy", service: "app-api" });
